...
var copyDir = function(cfg, callback) {
  ...
  if (inputPath && outputPath) {
    new gear.Queue({registry: taskRegistry}) // register tasks
    .log('\nCopying dir: '.cyan + inputPath.magenta)
    .copyDir({input: inputPath, output: outputPath}, callback)
    .log('Copied dir to: '.cyan + outputPath.magenta)
    .run(callback);
  } else {
    callback();
  }
...
var removeFiles = function(cfg, callback) {
  ...
  if (files.length > 0) {
    new gear.Queue({registry: taskRegistry}) // register tasks
    .log('\nRemoving files: '.cyan + files.join(', ').magenta)
...
var removeDir = function(cfg, callback) {
  ...
  if (inputPath) {
    new gear.Queue({registry: taskRegistry}) // register tasks
    .log('\nRemoving dir: '.cyan + inputPath.magenta)
    ...
    next: function(err,watcher){
      if (err) {
        ...
        if (cfg.input_files) {
          ...
          console.log('\nWatching setup successfully for dir: '.green + cfg.input_dir.magenta);
        }
        callback();
      }
    }
