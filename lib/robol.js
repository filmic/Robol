...
var buildScripts = function(cfg, callback) {
  ...
  if (jsFiles.length + coffeeFiles.length > 0) {
    ...
    .log('\nBuilding scripts: '.cyan + cfg.input_files.join(', ').magenta)
    .tasks({
...
var buildStyles = function(cfg, callback) {
  ...
  if (cssFiles.length+lessFiles.length > 0) {
    new gear.Queue({registry: taskRegistry}) // register tasks
    .log('\nBuilding styles: '.cyan + cfg.input_files.join(', ').magenta)
    .tasks({
...
var copyFiles = function(cfg, callback) {
  ...
  if (files.length > 0) {
    new gear.Queue({registry: taskRegistry}) // register tasks
    .log('\nCopying files: '.cyan + files.join(', ').magenta)
    .tasks({
...
var setupWatch = function(cfg, callback) {
  watchr.watch({
    ...
    listener: function(eventName,filePath,fileCurrentStat,filePreviousStat){
      console.log('\nFile changed:'.green, filePath.magenta);
      ...
      if (config.scripts.indexOf(cfg) !== -1) {
        ...
        // rebuild styles
        buildStyles(cfg);
      } else if (config.copyFiles.indexOf(cfg) !== -1) {
        // copy single file
        var singleFileCfg = {input_files: [filePath], output_dir: cfg.output_dir};
    ...
    next: function(err,watcher){
      if (err) {
        ...
        console.log('\nWatching setup successfully for files: '.green + cfg.input_files.join(', ').magenta);
        callback();
      }
    }
  });
...
exports.run = function(options) {
  ...
  async.auto({
    ...
    setup_watch: ['build_scripts', 'build_styles', 'copy_files', function(callback){
      if (options.w) {
        var filesToWatch = [].concat(config.scripts, config.styles, config.copyFiles);
